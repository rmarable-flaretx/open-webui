...
async def get_event_emitter(request_info, update_db=True):
    ...
    if (request_info.get('chat_id') or '').startswith('channel:'):  # fix: handle explicit None chat_id
    ...
    async def __event_emitter__(event_data):
        ...
        if update_db and message_id and not (request_info.get('chat_id') or '').startswith('local:'):  # fix: handle explicit None chat_id
            event_type = event_data.get('type')
